...
- Context Recall
  - 모범 답안에 포함된 핵심 정보가 얼마나 검색되었는지를 평가하는 지표입니다.
  - TopK 나 Chunk Size 변화에 따라 검색 범위가 달라질때 답변에 필요한 정보 누락이 얼마나 발생하는지 평가합니다.
- Context Precision
...
Chunk Size 를 적절한 수준으로 설정한 경우 문서가 개념이나 키워드가 임베딩 벡터에 효과적으로 반영되면서도, 각 청크 내부에 응답 생성에 필요한 정보가 충분하게 포함될 것으로 예상됩니다. 
...
Overlap이 증가함에 따라 동일한 정보가 여러 청크에 중복되어 포함될 가능성이 커집니다. 따라서 질문에 답하기 위한 핵심 정보가 여러 청크에 반복적으로 포함될 가능성이 높습니다.
